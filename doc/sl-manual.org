...
- Special Form: backquote expr :: <<backquote>>
  ...
  Return the argument, but allowing selective evaluation. Without using
  special /unquote/ arguments, described below, it's behavior is identical
  to =quote=.

  The backquote is pretty special, and can be really useful. For one, multiple symbols are
  bound to it in the environment. To call the backquote primitive, you
  can use either the =backquote= symbol or the =`= symbol[fn::That is, the
  /grave accent/ character, ASCII code 96.]. The =backquote= special form
  also handles other special symbols that are used for /unquoting/, that
  is, for evaluating parts of its argument.  An expression is unquoted
  if it's wrapped in a call to the =,= function[fn::That is, the /comma/
  character, ASCII code 44.]. Note that the symbol =,= is bound in the
  global environment, but cannot be used outside of a =backquote=
  argument.
...
- Function: equal? a b &rest rest :: <<equal?>>
  ...
  - Two non-empty lists are equal if they have the same number of
    ...
    function =equal?=.
  - Two numbers are equal if they share the same type, and if they have
    the same value. See also [[=][===]].
  - For symbols are equal if they have the same length, and if all of
    their characters match.
  - Strings are handled just like symbols, but note that, since they
    have different expression types, they are not equal according to
    this function.
...
- Function: < a b &rest rest :: <<lt>>
  ...
  $a<b<...<n$. Predicates are therefore transitive, that is, $a<c$.
  ...
  - Two expressions can only increase if they share the same type, so
    you can't compare integers and floats, or symbols and strings.
  - Two numbers are increasing or decreasing if the value of second is
    greater or smaller than the value of the first, respectively.
  - Two strings are increasing or decreasing if the first differing
    ...
    respectively.
  - Symbols are compared just like strings, but they can only be
    compared against other symbols.
...
- Function: > a b &rest rest :: <<gt>>
  ...
  $a>b>...>n$. Predicates are therefore transitive, that is, $a>c$. For
  more information on this function, see [[lt][=<=]].
