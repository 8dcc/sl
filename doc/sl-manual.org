...
them, they need to be internally converted to a /generic numeric
type/. Depending on the types and the values being converted, some
information might be lost in the process.

Currently, the generic numeric type is /Float/. For example, most
arithmetic function accept arguments of any numeric type, so if the list
of arguments is not homogeneous, they are converted and a generic
...
The C functions for handling generic numbers are defined as =inline= in
the [[file:expr.h]] header.
