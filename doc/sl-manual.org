...
- Variable: `*debug-trace*' :: <<*debug-trace*>>
  ...
  use the standard library function =trace= (see [[trace]]).
...
- Special Form: macro formals body... :: <<macro>>
  ...
  - Macros don't /directly/ compute values, they instead build Lisp
    ...
    [[macroexpand]]). In this step, the macro is called just like a
...
- Special Form: begin &rest exprs :: <<begin>>
  ...
  =apply= and a quoted expression (see [[apply]]).
...
- Function: apply function arg-list :: <<apply>>
  ...
  (e.g. =lambda_call=). For more information, see [[lambda]] and [[macro]].
...
- Function: equal? &rest args :: <<equal?>>
